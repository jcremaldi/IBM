...
def data_prep(df):
    ...
    temp = df[['Principal','terms','age','Gender','weekend']]
...
for n in range(1,Ks):
    ...
    mean_acc_knn[n-1] = strat_k_folds(neigh, X, y)
...
for n in range(1,dep):
    ...
    mean_acc_dt[n-1] = strat_k_folds(loanTree, X, y) 
...
neigh_pred = KNeighborsClassifier(n_neighbors = best_n)
neigh_pred.fit(X, y)
loanTree_pred = DecisionTreeClassifier(criterion="entropy", max_depth = best_dep)
loanTree_pred.fit(X, y)
clf_pred = svm.SVC(kernel=str(temp_max_svm))
clf_pred.fit(X, y)
LR_pred = LogisticRegression(C=best_c, solver=str(temp_max_lr))
LR_pred.fit(X, y)
